...
function ResetHistogram({ reasons }: { reasons: Record<string, number> }) {
  ...
  if (total === 0) return <p className="readout px-4 py-6 text-xs text-phosphor-dim">No reset data</p>;
  ...
  return (
    <div className="px-4 py-4">
      <div className="overflow-x-auto">
        ...
        */}
        <svg
          ...
          role="img"
        >
          <line x1={0} y1={chartH + topPad} x2={svgW} y2={chartH + topPad} stroke="#1d3a3a" strokeWidth={1} />
          {entries.map(([reason, count], i) => {
            ...
            return (
              <g key={reason}>
                <text x={x + barW / 2} y={y - 5} textAnchor="middle" fill="#7ff3e0" fontSize="10" fontFamily="var(--font-readout)">
                  {count}
                </text>
                ...
                <text x={x + barW / 2} y={chartH + topPad + 16} textAnchor="middle" fill="#4a8f86" fontSize="9" fontFamily="var(--font-readout)">
                  #{reason}
                </text>
              </g>
            );
      ...
      <p className="readout mt-4 text-[11px] text-phosphor-dim/80">
...
function HeapGauge({ free, largest }: { free: number; largest: number }) {
  ...
  const tone = freePct > 30 ? '#7ff3e0' : freePct > 15 ? '#4a8f86' : '#ff6b5a';

  return (
    <div className="px-4 py-4">
      <div className="mb-3 flex items-end gap-4">
        <div className="readout bloom text-4xl leading-none text-phosphor">
          {Math.round(free / 1024)}
          <span className="ml-1.5 text-[0.4em] text-phosphor-dim">KB free</span>
        </div>
        <div className="readout pb-1 text-xs text-phosphor-dim">
      ...
      <p className="readout mt-2 text-[11px] text-phosphor-dim">
