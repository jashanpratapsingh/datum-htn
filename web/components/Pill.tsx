...
const base =
  ...
  'px-4 sm:px-5 py-[0.35em] mx-[0.2em] mb-[0.4em] whitespace-nowrap ' +
  'font-[family-name:var(--font-readout)] transition-colors duration-200';

/** A solid readout pill. Links to somewhere that actually exists. */
export function Pill({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <a
      href={href}
      className={`${base} bg-phosphor text-glass border border-phosphor hover:bg-transparent hover:text-phosphor`}
    >
      {children}
    </a>
  );
...
/**
 * Outline pill that copies a value.
 *
...
export function CopyPill({ value, label }: { value: string; label: string }) {
  ...
  return (
    <button
      ...
      className={`${base} gap-2 sm:gap-3 border border-phosphor/45 bg-transparent text-phosphor/85 hover:bg-phosphor hover:text-glass`}
    >
      <span className="underline underline-offset-2 decoration-phosphor/40">
        {copied ? 'copied' : label}
      </span>
      <svg
        width="12"
